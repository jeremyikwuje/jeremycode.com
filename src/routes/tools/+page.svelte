...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
	...
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
		<div>
			...
			>
				{filterStore.state.vertical ? `${filterStore.state.vertical.name} Tools` : 'All Tools'}
			</h1>
		...
		<div class="hidden sm:block w-64">
			<SearchInput mode="inline" placeholder="Search tools…" />
		</div>

		<!-- Mobile: filter button + search -->
		<div class="flex items-center gap-3 sm:hidden">
			<SearchInput mode="inline" class="flex-1" />
			<button
				type="button"
				onclick={() => uiStore.openFilterDrawer()}
				class="shrink-0 inline-flex items-center gap-1.5 h-11 px-3 text-sm font-medium
